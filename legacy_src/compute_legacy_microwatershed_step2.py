...
print("threshold is 10000")
# Load the rasters using rasterio
from flow_direction import load_tif_image
from make_tif import GeoTIFFHandler
print("handling masalia watersheds on old qgis data")

FLOW_DIR='../tifs/dem_experiments/doubleCheckResults/masalia_flow_dir_from_SRTM_Cupy.tif'
FLOW_ACC='../tifs/dem_experiments/doubleCheckResults/flow_acc_masalia_cupy_from_SRTM.tif'
OUTPUT='../tifs/dem_experiments/doubleCheckResults/masalia_microwatershed_cupy_SRTM_10k.tif'
# FLOW_DIR='../tifs/masalia/qgis/qgis_masalia_fdir.tif'
# FLOW_ACC='../tifs/masalia/experiment_tifs/masalia_f_acc.tif'
# OUTPUT='../tifs/dem_experiments/actual_masalia_microwatershed_from_qgis_facc_cupy.tif'
handler = GeoTIFFHandler(FLOW_DIR)

# # Load data as NumPy and convert to CuPy
flow_direction = cp.asarray(load_tif_image(FLOW_DIR))
flow_accumulation = cp.asarray(load_tif_image(FLOW_ACC))
flow_accumulation_stream_cells=(flow_accumulation>=stream_threshold_initial).astype(cp.int32)
...
initial_basins_cp = calculate_microwatersheds_by_convergence(flow_direction, flow_accumulation_stream_cells)

handler.save_tiff(cp.asnumpy(initial_basins_cp).astype(np.float32), OUTPUT)

# handler = GeoTIFFHandler('../tifs/dem_experiments/dem_depless_masalia_clipped.tif')

# # # Load data as NumPy and convert to CuPy
# flow_direction = cp.asarray(load_tif_image('../tifs/dem_experiments/masalia_fd_without_clipping.tif'))
# flow_accumulation = cp.asarray(load_tif_image('../tifs/dem_experiments/flow_acc_masalia_cupyFd.tif'))
# print("lgb 1000")
# print(f"Starting initial basin delineation with outlet threshold: {stream_threshold_initial}")
# # Calculate initial basins (IDs based on outlet cell index)
# start_time = time.time()
# initial_basins_cp = calculate_microwatersheds_by_convergence(flow_direction, flow_accumulation)
# print("micro watershed complition time : ", time.time() - start_time)

# handler.save_tiff(cp.asnumpy(initial_basins_cp).astype(np.float32),  '../tifs/dem_experiments/masalia_mwshed_new.tif')
...
# handler = GeoTIFFHandler('../tifs/lgb/flow_dir.tif')

# # # Load data as NumPy and convert to CuPy
# flow_direction = cp.asarray(load_tif_image('../tifs/lgb/flow_dir.tif'))
# flow_accumulation = cp.asarray(load_tif_image('../tifs/lgb/flow_acc.tif'))
# flow_accumulation_stream_cells=(flow_accumulation>=stream_threshold_initial).astype(cp.int32)

# print("computing on lgb")
# print(f"Starting initial basin delineation with outlet threshold: {stream_threshold_initial}")
# # Calculate initial basins (IDs based on outlet cell index)
# start_time=time.time()
# initial_basins_cp = calculate_microwatersheds_by_convergence(flow_direction, flow_accumulation_stream_cells)
# print("mwsheds computed in ",time.time()-start_time)
# handler.save_tiff(cp.asnumpy(initial_basins_cp).astype(np.float32),  '../tifs/lgb/microwatershed.tif')
